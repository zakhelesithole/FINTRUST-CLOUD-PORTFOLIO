# ============================================
# FinTrust Bank — Fixed Transaction Processor
# Week 4 Day 2 PM Lab — All 5 bugs fixed
# ============================================
...
def calculate_fee(amount):
    ...
    fee = amount * 0.005              # FIX 1: 0.005 = 0.5%
    return max(fee, 5.0)
...
def process_payment(sender_id, receiver_id, amount):
    ...
    # Process
    sender["balance"] -= total_deducted    # FIX 2: 'balance' not 'balanse'
...
# FIX 3: use correct key 'balance'
print(f"\nFT-001 final balance: R{ACCOUNTS['FT-001']['balance']:.2f}")

# FIX 4: only sum the amount (fee is already tracked separately)
total_sent = sum(r["amount"] for r in results)
print(f"Total processed (excl. failed): R{total_sent:.2f}")

# FIX 5: failed count = total payments - successful results
